...
def test_the_post_debate_chain_is_still_linear():
    graph = build_trading_graph(InMemorySaver()).get_graph()
    linear = {(e.source, e.target) for e in graph.edges if not e.conditional}

    assert ("debate_close", "risk") in linear
    assert ("risk", "synthesizer") in linear
    assert ("synthesizer", "__end__") in linear


def test_the_old_debate_stub_node_is_gone():
    """Pre-Phase-5 threads are unresumable because this name no longer
    exists in the graph. Fresh --thread-id for all Phase 5 work."""
    graph = build_trading_graph(InMemorySaver()).get_graph()
